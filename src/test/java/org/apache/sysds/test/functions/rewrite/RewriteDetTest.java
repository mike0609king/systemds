...
public class RewriteDetTest extends AutomatedTestBase 
{
	...
	private void runRewriteDetTest(String testScriptName, boolean rewrites, boolean sparse, ExecType et) {
		...
		try {
			...
			boolean twoMatrixArg = testScriptName.equals(TEST_NAME_MULT) || testScriptName.equals(TEST_NAME_MIXED);
			if (twoMatrixArg) {
			...
			if (rewrites) {
				Assert.assertTrue(
					...
					&& (!testScriptName.equals(TEST_NAME_MIXED) || (
						Statistics.getCPHeavyHitterCount("det") == 2 && !heavyHittersContainsString("r'"))));
			}
		}
		finally {
